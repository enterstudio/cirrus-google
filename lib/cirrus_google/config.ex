defmodule Cirrus.Google.Config do
  use GenServer
  ...
  def init(:ok) do
    case Application.get_env(:cirrus_google, :json) do
      nil ->
        {:ok, Application.get_env(:cirrus_google, :config, %{})}
      json ->
        {:ok, Poison.decode!(json)}
    end
  end

  def set(key, value) when is_atom(key),
    do: key |> to_string |> set(value)
